...
void Window::on_OK_clicked()
{
    ...
    if (log !="" and password !="")
    {
        QString message = "auth&"+log+"&"+password;
...
void Window::EducationTrue(QString data)
{
    ui->winalgoritm1->setVisible(false);
    ui->winalgoritm2->setVisible(false);
    ui->winreport->setVisible(true);

    ui->answer->insertPlainText(data);
...
{
    std::vector<std::string> truepass = {{"ABCDEFGHIJKLMNOPQRSTUVWXYZ"}, {"abcdefghijklmnopqrstuvwxyz"}, {"1234567890"}, {"-_!@#$%^*<>?"}}; // разрешенные символы
...
/*//////////////////////////\\|***|MYTCPCLIENT|***|\\//////////////////////////////////////*/
    ...
    void Window::slot_readyRead()
    {
        ...
        std::string answer = message.substr(0,pos);
        message.erase(0,pos+1);
        if (answer == "auth" and message =="1")
        {
            Auth();
        }
        else if (answer == "auth" and message =="0")
        {
            FailAuth();
            qDebug() << "fail auth";

        }
        else if (answer == "reg" and message =="1")
        {
            Reg();
        }

        else if (answer == "reg" and message =="0")
        {
            qDebug() << "fail reg";
            FailReg();
        }
        ...
        {
            qDebug() << "Successful training";
            EducationTrue(QString::fromStdString(message));
        }
        else qDebug() << "fail";
